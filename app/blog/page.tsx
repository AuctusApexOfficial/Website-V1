                ...
                {currentPosts.map((post, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      whileHover={
                        isWebDesignArticle || isSecondPosition ? { y: -10, transition: { duration: 0.3 } } : {}
                      }
                      className={`group bg-white rounded-lg shadow-sm ${isWebDesignArticle || isSecondPosition ? "hover:shadow-xl" : ""} transition-all duration-500 relative overflow-hidden`}
                      onHoverStart={() => (isWebDesignArticle || isSecondPosition ? setHoveredPost(post.id) : null)}
                      onHoverEnd={() => (isWebDesignArticle || isSecondPosition ? setHoveredPost(null) : null)}
                    >
                      <div
                        className={`absolute inset-0 bg-gradient-to-br from-gold-200/0 to-gold-300/10 opacity-0 ${isWebDesignArticle || isSecondPosition ? "group-hover:opacity-100" : ""} transition-opacity duration-500`}
                      ></div>
                      <div
