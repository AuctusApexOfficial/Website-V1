...
export default function BlogPage() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<"all" | "en" | "it">("all")
  ...
  // Filter posts based on category, search query, and language
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Handle scrolling for preview elements
    const setupScrolling = (elementId: string) => {
      const previewElement = document.getElementById(elementId)
      if (!previewElement) return

      let scrollInterval: NodeJS.Timeout | null = null
      let isPaused = true // Start with scrolling paused
      let startY = 0
      ...
      // Start scrolling initially but paused
      startScrolling()
      ...
      const handleMouseUp = () => {
        // Only resume auto-scroll if we're still hovering
        if (isHovering) {
          isPaused = false
        }
      }

      // Add hover event listeners to control scrolling
      ...
      return () => {
        ...
        previewElement.removeEventListener("mouseenter", handleMouseEnter)
        document.removeEventListener("mouseleave", handleMouseLeave)
      }
    }

    // Setup scrolling for all preview elements
    const webDesignCleanup = setupScrolling("web-design-preview")
    const aiMarketingCleanup = setupScrolling("ai-marketing-preview")
    const brandDesignCleanup = setupScrolling("brand-design-preview") // Add this for the brand design article

    // Return combined cleanup
    return () => {
      if (webDesignCleanup) webDesignCleanup()
      if (aiMarketingCleanup) aiMarketingCleanup()
      if (brandDesignCleanup) brandDesignCleanup()
    }
  }, [])
        ...
        <div className="container mx-auto px-4 relative pt-16">
          ...
          >
            <div className="mx-auto mb-4 flex max-w-xs justify-center">
              ...
              <p className="mx-4 font-serif text-sm uppercase tracking-widest text-gold-700">
                {language === "en" ? "Our Insights" : language === "it" ? "Tutti" : "Todos"}
              </p>
              <div className="h-[1px] w-16 self-center bg-gold-600"></div>
            </div>
          ...
          >
            <div className="inline-flex items-center p-1 bg-white/80 backdrop-blur-sm rounded-full border border-gold-200 shadow-sm">
              <button
                onClick={() => setSelectedLanguage("all")}
                className={`px-6 py-2 text-sm font-serif transition-all rounded-full ${
                  selectedLanguage === "all" ? "bg-gold-600 text-white shadow-sm" : "text-taupe-700 hover:bg-gold-50"
                }`}
              >
                {language === "en" ? "All" : language === "it" ? "Tutti" : "Todos"}
              </button>
              <button
                onClick={() => setSelectedLanguage("en")}
                className={`px-6 py-2 text-sm font-serif transition-all rounded-full ${
                  selectedLanguage === "en" ? "bg-gold-600 text-white shadow-sm" : "text-taupe-700 hover:bg-gold-50"
                }`}
              >
                English
              </button>
              <button
                ...
                className={`px-6 py-2 text-sm font-serif transition-all rounded-full ${
                  selectedLanguage === "it" ? "bg-gold-600 text-white shadow-sm" : "text-taupe-700 hover:bg-gold-50"
                }`}
              >
                Italiano
              </button>
            ...
            <>
              ...
              >
                {currentPosts.map((post, index) => {
                  ...
                  const isSecondPosition = index === 1 // Identify the second position post
                  // First, add a new variable to identify the AI Marketing article
                  const isAIMarketingArticle = post.id === 2
                  ...
                  return (
                    <motion.div
                      ...
                      whileHover={
                        isWebDesignArticle || isSecondPosition || isAIMarketingArticle
                          ? { y: -10, transition: { duration: 0.3 } }
                          : {}
                      }
                      className={`group bg-white rounded-lg shadow-sm ${isWebDesignArticle || isSecondPosition || isAIMarketingArticle ? "hover:shadow-xl" : ""} transition-all duration-500 relative overflow-hidden`}
                      // Then, update the hover handlers to include the AI Marketing article
                      onHoverStart={() =>
                        isWebDesignArticle || isSecondPosition || isAIMarketingArticle ? setHoveredPost(post.id) : null
                      }
                      onHoverEnd={() =>
                        isWebDesignArticle || isSecondPosition || isAIMarketingArticle ? setHoveredPost(null) : null
                      }
                    >
                      <div
                        // Update the gradient div to include the AI Marketing article
                        className={`absolute inset-0 bg-gradient-to-br from-gold-200/0 to-gold-300/10 opacity-0 ${
                          isWebDesignArticle || isSecondPosition || isAIMarketingArticle
                      ...
                      <div className="aspect-[16/9] overflow-hidden rounded-t-lg bg-gray-100">
                        {isWebDesignArticle ? (
                          ...
                          >
                            ...
                            <p className="mb-3">
                              The concept of "timeless design" presents an inherent paradox: how can something remain
                              relevant across changing eras, technologies, and cultural contexts? The answer lies in
                              understanding which elements should evolve and which should remain constant.
                            </p>
                            <h4 className="font-serif text-md font-medium text-taupe-800 mb-1">
                              Foundational Elements
                            </h4>
                            <p className="mb-3">
                              Creating a brand identity with staying power requires careful attention to simplicity,
                              typography, color selection, and meaningful symbolism that transcends trends.
                            </p>
                            <h4 className="font-serif text-md font-medium text-taupe-800 mb-1">Strategic Balance</h4>
                            <p className="mb-3">
                              While timelessness is desirable, brands must also feel current and connected to
                              contemporary culture. This balance can be achieved through a strategic approach to brand
                              architecture.
                            </p>
                        ...
                        ) : isAIMarketingArticle ? (
                          <div
                            id="ai-marketing-preview"
                            className="w-full h-full overflow-y-auto bg-white p-4 text-sm text-taupe-700 scrollbar-hide"
                            style={{ maxHeight: "300px" }}
                          >
                            <h3 className="font-serif text-lg font-medium text-taupe-900 mb-2">
                            ...
                            <p className="mb-3">
                              ...
                              data exploding, attention spans shrinking, and competition tightening by the minute, the
                              old ways of doing business simply don't cut it anymore.
                            </p>
                            <h4 className="font-serif text-md font-medium text-taupe-800 mb-1">
                              The AI Shift in Marketing
                            </h4>
                            <p className="mb-3">
                              Artificial Intelligence in marketing has evolved from novelty to necessity. Once used
                              primarily for chatbots or basic automations, AI is now embedded in every stage of the
                              funnel: research, creation, distribution, analysis, and optimization.
                            </p>
                            <h4 className="font-serif text-md font-medium text-taupe-800 mb-1">
                              Intelligent Content Creation
                            </h4>
                            <p className="mb-3">
                              Marketing starts with content. At Auctus, we use advanced language models to generate
                              conversion-optimized landing pages, social media scripts, ad copy, and email sequences at
                              scale. These aren't templates—they're tailored assets, informed by audience data and
                              performance history.
                            </p>
                            <h4 className="font-serif text-md font-medium text-taupe-800 mb-1">
                              Hyper-Automated Lead Generation
                            </h4>
                            <p className="mb-3">
                              Lead generation is no longer just about quantity. Precision matters. We use AI platforms
                              to mine thousands of potential leads, qualify them by firmographic and behavioral data,
                              and generate dynamic email campaigns tailored to each segment.
                            </p>
                            <div className="mt-2 pt-2 border-t border-gold-100">
                              <div className="flex justify-between items-center">
                      ...
                      <div className="p-6 relative">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {/* Then, in the section where we display the article details and "Read" button, add a condition for the AI Marketing article */}
                          {isWebDesignArticle && (
                            <Link
                              href="/blog/importance-of-web-design"
                          ...
                          {isAIMarketingArticle && (
                            <Link
                              href="/blog/leveraging-ai-modern-marketing"
                              className="inline-flex items-center text-gold-600 hover:text-gold-700 transition-colors"
                              target="_blank"
